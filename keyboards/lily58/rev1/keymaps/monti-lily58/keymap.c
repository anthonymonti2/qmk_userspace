...
// Forward defs
bool is_any_layer_locked(layer_state_t locked_layers);
...
// Mapping from layer number to layer name for display on OLED
static const char *PROGMEM LAYER_NAME[] = {
    "COLE", "QWRTY", "CGAME", "QGAME", "NAV", "NUM", "MEDIA", "SYMB", "FUNC"
};

static_assert(
    (sizeof(LAYER_NAME) / sizeof(LAYER_NAME[0])) == LAYER_NUMBER_MAX,
    "The number of layer names does not match LAYER_NUMBER_MAX!"
);

/* Display functions*/
/**
 * @brief Get the current active layer.
 * @return Returns the highest active layer number.
 */
...
/**
 * @brief Get the name of the current layer in uppercase letters.
 * @param layer The layer number to get the name for.
 * @return Returns the name of the current layer in uppercase letters, or "UNDEFINED" if the layer is not defined.
 */
 ...
 */                 
 ...
 [_QWERTY] = LAYOUT( \
  ...
  _______,   KC_Z,   KC_X,    KC_C,    KC_D,    KC_V, KC_LBRC,  KC_RBRC,  KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH, KC_RSFT, \
...
[NAV] = LAYOUT(
    ...
    U_NU,   KC_LGUI, KC_LALT, KC_LCTL, KC_LSFT, QK_LLCK,            KC_CAPS, KC_LEFT, KC_DOWN, KC_RGHT, U_COPY, U_NU, \
...
[NUM] = LAYOUT(
    ...
    U_NU,   KC_SCLN, KC_4,    KC_5,    KC_6,    KC_EQL,                 QK_LLCK, KC_LSFT, KC_LCTL, KC_LALT, KC_LGUI, U_NU, \
...
[MEDIA] = LAYOUT(
    ...
    U_NU,   KC_LGUI, KC_LALT, KC_LCTL, KC_LSFT, QK_LLCK,                U_NU,    KC_MPRV, KC_VOLD, KC_VOLU, KC_MNXT, U_NU, \
...
[SYM] = LAYOUT(
    ...
    KC_BSLS,  KC_EXLM, KC_UNDS, KC_PLUS, KC_EQL, KC_ASTR,                  QK_LLCK,   KC_LSFT, KC_LCTL, KC_LALT, KC_LGUI, U_NU, \
...
[FUN] = LAYOUT(
    ...
    U_NU,    KC_F11,  KC_F4,   KC_F5,   KC_F6,   KC_APP ,                   QK_LLCK, KC_LSFT,       KC_LCTL,    KC_LALT,    KC_LGUI, U_NU,\
...
void render_status_main(void) {
    ...
    render_on_off_status("LCK", a_layer_is_locked);
...
// I think is now automatically handled by QMK, but leaving this here for now in case it is needed
...
// Called every time the layer lock key is pressed
bool layer_lock_set_user(layer_state_t locked_layers) {
    // Save the layer lock state to a global variable so we can use it in the oled task
    a_layer_is_locked = is_any_layer_locked(locked_layers);
    return true;
}
